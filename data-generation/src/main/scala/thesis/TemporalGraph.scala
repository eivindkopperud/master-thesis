...
abstract class TemporalGraph extends Serializable {
  // Removing these since they were not necessary or not implemented
  // They should in theory return the graph in its current form
  // TemporalGraph.snapshotAtTime(Instant.now)

...
class Landy(graph: LandyAttributeGraph) extends TemporalGraph {
  val vertices: VertexRDD[LandyEntityPayload] = graph.vertices
  val edges: EdgeRDD[LandyEntityPayload] = graph.edges
  ...
  def snapshotAtTimeLandy(instant: Instant): Graph[LandyEntityPayload, LandyEntityPayload] = {
    ...
    val edges = this.graph.edges.filter(edge =>
  ...
  private def localVertices: VertexRDD[LandyEntityPayload] = {
    this.graph.vertices.filter(vertex => vertex._2 != null)
  }
